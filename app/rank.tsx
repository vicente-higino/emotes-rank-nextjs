...
import { Button, CheckboxGroup, Dialog, Flex, Grid, Heading, IconButton, Link, Section, Select, Skeleton, Spinner, Switch, Text } from "@radix-ui/themes";
import { type DateRange, DayPicker } from "react-day-picker";
import { Emote, parseProviders, EmoteProvider, getEmotes, getEmoteUrl, getEmotePageUrl, ProviderColor, normalizeDateRange } from "./util";
...
function reducer(state: State, action: Action): State {
  ...
  switch (action.type) {
    ...
    case "SET_OPEN":
      return {
        ...newState, filterDateRange: action.open ?
          state.filterDateRange : normalizeDateRange(state.dateRangeSelection?.from, state.dateRangeSelection?.to),
        dateRangeSelectionDiaglogOpen: action.open
      };
    case "SET_ONLY_CURRENT":
      if (action.value) {
        return {
          ...newState, onlyCurrentEmotes: action.value, page: 1, totalPages: 1,
...
export default function RankPage() {
  ...
  const [state, dispatch] = useReducer(reducer, {
    ...
    filterDateRange: normalizeDateRange(),
    month: undefined,
    providerFilter: searchParams.get("providers") ? parseProviders(searchParams.get("providers")!.split(",")).toSorted() : Object.values(EmoteProvider).toSorted(),
    dateRangeSelectionDiaglogOpen: false,
    onlyCurrentEmotes: false,
  });
  ...
  // const queryProviders = useMemo(
  //   () => [...state.providerFilter].sort(),
  //   [state.providerFilter]
  // );

  const { isLoading, isError, data, error, refetch } = useQuery({
    queryKey: ['emotes', state.channel, state.page, state.perPage, state.filterDateRange?.from, state.filterDateRange?.to, queryFilter.onlyCurrentEmotes, ...queryFilter.providerFilter],
    queryFn: () => getRows(),
  })
      ...
      <Grid gap="2" align="center" justify="center" style={{ height: "100%", width: "100%", gridColumn: '1/-1' }}>
        <Flex direction="column" align="center" gap="2">
          {isLoading ? <Spinner size="3" /> : <>
            <Text size="4" weight="bold">No Emotes Found</Text>
  ...
  return (<>
    <Section size={"1"}>
      <Flex direction={"column"} gap={"3"} align={"center"}>
        <Heading>Channel: {state.channel}</Heading>
        <Flex gap="2" justify={"center"}>
          <Select.Root
            disabled={isLoading}
            defaultValue="fuslie"
            value={state.channel}
            onValueChange={v => dispatch({ type: "SET_CHANNEL", channel: v })}>
            <Select.Trigger />
            <Select.Content>
              <Select.Item value="fuslie">fuslie</Select.Item>
              <Select.Item value="fukura____">fukura____</Select.Item>
              <Select.Item value="v_cn_t">v_cn_t</Select.Item>
            </Select.Content>
          </Select.Root>
          ...
          {/* {state.hasChanged && (
            <Button color="red" loading={isLoading} onClick={() => refetch()}>
              Refresh
            </Button>
          )} */}
        </Flex>
          ...
          <Dialog.Root open={state.dateRangeSelectionDiaglogOpen} onOpenChange={v => dispatch({ type: "SET_OPEN", open: v })} >
            <Dialog.Trigger>
              <Button disabled={isLoading} >
                <CalendarIcon /> {state.dateRangeSelection ? (
                  <>
                    {state.dateRangeSelection.from?.toLocaleDateString()} <DoubleArrowRightIcon /> {state.dateRangeSelection.to?.toLocaleDateString()}
                  </>
                ) : (
                  "Select Range"
                )}
        ...
        <Flex gap={"5"} align={"center"}>
          ...
          <IconButton
            ...
            onClick={() => dispatch({ type: "SET_PAGE", page: state.page + 1 })} variant="ghost">
            <ChevronRightIcon />
          </IconButton>

          <IconButton
            disabled={state.page === state.totalPages || isLoading}
            onClick={() => dispatch({ type: "SET_PAGE", page: state.totalPages })} variant="ghost">
